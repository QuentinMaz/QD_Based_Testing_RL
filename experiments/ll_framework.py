...
from ll_common import load_lunar_lander_model, DEFAULT_MIN, DEFAULT_MAX, execute_policy, get_edges
from common import compute_cell, EXPERIMENT_SEEDS

class Framework():
    def __init__(self, rand_seed: int, cell_granularity: int = 50, **kwargs) -> None:
        self.version = 'random'
    ...
    def mutate(self, input: np.ndarray, lows: List[int] = [DEFAULT_MIN, DEFAULT_MIN], highs: List[int] = [DEFAULT_MAX, DEFAULT_MAX], intensity: float = 5.0):
        return np.clip(self.rng.normal(input, intensity), lows, highs)
    ...
    def test_policy(self, model: BaseAlgorithm,
        ...
        for _ in tqdm.tqdm(range(init_budget), disable=disable_pbar):
            ...
            t0 = time.time()
            episode_reward, oracle, behavior, fs, _ = execute_policy(input, model, env_seed, 1000)
            t1 = time.time()
        ...
        self.xedges, self.yedges = get_edges(env_seed, 1000)

        self.config['xedges'] = list(self.xedges)
        self.config['yedges'] = list(self.xedges)

        # computes cell edges with the initial behaviors recorded
        # _H, init_xedges, init_yedges = np.histogram2d(behaviors[:, 0], behaviors[:, 1], bins=self.granularity)
        ...
        while (current_time - start_time < time_budget) and (nb_executions < executions_budget):
            ...
            t0 = time.time()
            episode_reward, oracle, behavior, fs, _ = execute_policy(mutated_input, model, env_seed, 1000)
            t1 = time.time()
            execution_times.append(t1 - t0)
            cell = compute_cell(behavior, self.xedges, self.yedges).tolist()
            ...
            np.savetxt(inputs_buffer, mutated_input.reshape(1, -1), delimiter=',')
            np.savetxt(behaviors_buffer, behavior.reshape(1, -1), delimiter=',')
    ...
    def novelty_search(self, model: BaseAlgorithm,
        ...
        def evaluate(individuals: np.ndarray) -> np.ndarray:
            ...
            for ind in individuals:
                r, o, b, fs, _ = execute_policy(ind, model, env_seed, 1000)
                record(ind, r, o, b, fs)
                behaviors.append(b)
        ...
        pop = self.rng.uniform(low=DEFAULT_MIN, high=DEFAULT_MAX, size=(pop_size, 2))
...
class MAPElitesFramework(Framework):
    def __init__(self, rand_seed: int, cell_granularity: int = 50, **kwargs) -> None:
        ...
        super().__init__(rand_seed, cell_granularity, **kwargs)
...
if __name__ == '__main__':
    ...
    k = 3
    novelty_threshold = 0.005 # 0.004 makes the archive bigger and the number of faults lower

    results_fp = 'results/ll/'
    if not os.path.isdir(results_fp):
        os.mkdir(results_fp)

    for seed in EXPERIMENT_SEEDS:
        # baseline: Random Testing
        f = Framework(seed, cell_granularity, name='Random Testing')
        f.random_testing(model, env_seed, test_budget, results_fp)
        # QD-baseline: MAP-Elites
        f = MAPElitesFramework(seed, cell_granularity, name='MAP-Elites')
        f.test_policy(model, env_seed, test_budget, init_budget, results_fp)
        # D-baselines: Novelty Search
        f = Framework(seed, cell_granularity, name='Novelty Search')
        f.novelty_search(
            model, env_seed,
            population_size,
            nb_iterations,
