...
from ll_common import FEATURES, load_lunar_lander_model, DEFAULT_MIN, DEFAULT_MAX, execute_stochastic_policy, get_edges
from common import ENV_SEEDS, compute_cell, EXPERIMENT_SEEDS, get_bin_edges
...
class Framework():
    ...
    def test_policy(self, model: BaseAlgorithm,
        ...
        for _ in tqdm.tqdm(range(init_budget), disable=disable_pbar):
            ...
            t0 = time.time()
            episode_reward, failure_prob, final_obs_list, measures = execute_stochastic_policy(input, model, env_seeds, 1000)
            t1 = time.time()
        ...
        df = pd.read_csv("grid/ll/measures.csv")
        ...
        while (current_time - start_time < time_budget) and (nb_executions < executions_budget):
            ...
            t0 = time.time()
            episode_reward, failure_prob, final_obs_list, measures = execute_stochastic_policy(mutated_input, model, env_seeds, 1000)
            t1 = time.time()
            execution_times.append(t1 - t0)
    ...
    def novelty_search(self, model: BaseAlgorithm,
        ...
        def evaluate(individuals: np.ndarray) -> np.ndarray:
            ...
            for ind in individuals:
                r, fp, final_obs_list, measures =  execute_stochastic_policy(ind, model, env_seeds, 1000)
                fs = final_obs_list[0] #TODO
