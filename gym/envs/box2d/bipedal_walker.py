...
#TODO
class BipedalWalkerV4(gym.Env, EzPickle):
    metadata = {"render.modes": ["human", "rgb_array"], "video.frames_per_second": FPS}
    ...
    def __init__(self, rand_seed: int):
        EzPickle.__init__(self)
        self.rand_seed = rand_seed #todo
        self.rng = np.random.default_rng(rand_seed) #todo
        self.rand_history = [] #todo
        self.nb_states_used = 0
        self.use_indices = []
        self.use_terrain_locations = []
        self.use_x_locations = []
        # self.seed() todo
        self.viewer = None
    ...
    # def seed(self, seed=None): #todo
        # self.rng, seed = seeding.rng(seed)
        # print(seed)
        # return [seed]
    ...
    def _generate_terrain(self, hardcore, states):
        ...
        for i in range(TERRAIN_LENGTH):
            ...
            if state == GRASS and not oneshot:
                ...
                tmp = self.rng.integers(4, 5)
                tmp1 = self.rng.integers(3, 5)
                self.rand_history.append(tmp)
                self.rand_history.append(tmp1)
                stair_width = tmp
                stair_steps = tmp1
                original_y = y
                for s in range(stair_steps):
            ...
            counter -= 1
            if counter == 0:
                tmp = self.rng.integers(TERRAIN_GRASS / 2, TERRAIN_GRASS)
                self.rand_history.append(tmp)
                counter = tmp

                if states is None:
                    if state == GRASS and hardcore:
                        tmp = self.rng.integers(1, _STATES_)
                        self.rand_history.append(tmp)
                        state = tmp
                        oneshot = True
                    else:
                    ...
                    if state == GRASS and hardcore:
                        ...
                        self.use_indices.append(index_of_state)
                        state = states[index_of_state]
                        oneshot = True
                        index_of_state += 1
                        if index_of_state >= len(states):
    ...
    def _generate_clouds(self):
        ...
        self.cloud_poly = []
        for i in range(TERRAIN_LENGTH // 20):
            tmp = self.rng.uniform(0, TERRAIN_LENGTH)
            self.rand_history.append(tmp)
            x = tmp * TERRAIN_STEP
            y = VIEWPORT_H / SCALE * 3 / 4
            poly = []
            for a in range(5):
                tmp = self.rng.uniform(0, 5 * TERRAIN_STEP)
                tmp1 = self.rng.uniform(0, 5 * TERRAIN_STEP)
                self.rand_history.append(tmp)
                self.rand_history.append(tmp1)
                poly.append(
                (
            ...
            #         x
            #         + 15 * TERRAIN_STEP * math.sin(3.14 * 2 * a / 5)
            #         + self.rng.uniform(0, 5 * TERRAIN_STEP),
            #         y
            #         + 5 * TERRAIN_STEP * math.cos(3.14 * 2 * a / 5)
            #         + self.rng.uniform(0, 5 * TERRAIN_STEP),
            #     )
    ...
    def reset(self, states=None):
        self.rng = np.random.default_rng(self.rand_seed) #todo
        self.rand_history = []
        self._destroy()
        self.world.contactListener_bug_workaround = ContactDetector(self)
        self.world.contactListener = self.world.contactListener_bug_workaround
        ...
        self.hull.color1 = (0.5, 0.4, 0.9)
        self.hull.color2 = (0.3, 0.3, 0.5)
        tmp = self.rng.uniform(-INITIAL_RANDOM, INITIAL_RANDOM)
        self.rand_history.append(tmp)
        self.hull.ApplyForceToCenter(
            (tmp, 0), True
        )
